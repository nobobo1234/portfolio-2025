<script lang="ts">
	...
	let contentJson = $state(post.content);
	...
	function handleEditorCreate(event: EditorEvents['create']) {
		let parsed: unknown;
		try { parsed = JSON.parse(post.content); } catch { parsed = null; }
...
<div class="edit-post">
	<SmallTitle>edit blog post</SmallTitle>
	...
	>
		...
		<section class="form-section">
			...
			<div class="field-row">
				<label class="field">
					<span>Title</span>
					<input type="text" name="title" value={v?.title ?? post.title} required maxlength={200} />
				</label>
				<label class="field">
					...
					<input
						type="date"
						name="publishedAt"
						value={v?.publishedAt ?? toDateInputValue(post.publishedAt)}
						required
					/>
				</label>
			</div>
